...
public class RegularMatch {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new RegularMatch().isMatch2(s, 0, m, 0));
    }
    ...
    private boolean isMatch2(String s, int i, String m, int j) {
        //不能使用m-l
        if (j == m.length())
            return i == s.length();
        ...
        if (m.charAt(j + 1) != '*') {
            //  i<s.length() 防止越界
            return isMatch(s.charAt(i), m.charAt(j)) && isMatch(s, i + 1, m, j + 1);
        }

        return isMatch2(s, i, m, j + 2) || (isMatch2(s, i + 1, m, j) && isMatch(s.charAt(i),m.charAt(j)));
